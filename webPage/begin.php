...
<!--
    ...
    $_GET['courseId']:array of classes.
-->
<html>
<head>
    <title>选课结果</title>
...
$planHalfB = [];
$planCount = 0;
$Max_Conflict = (int) (2 * (float) $_GET['MaxConflict']);
$N = sizeof($_GET['selectedCS']);
$selectedLessons = [];
function searchLesson($curIndex, $sumHalfA, $sumHalfB, $conflictNum)
{
    ...
    if ($curIndex == $N) {
        //到达这一步就说明找到了一个新的选课方案
        $planRecord[] = [];
        foreach ($selectedLessons as $sels) {
            $planRecord[$planCount][] = $sels;
        }
    ...
    for ($i = 0; $i < sizeof($_GET[$_GET['selectedCS'][$curIndex]]); ++$i) {
        $lsId = $_GET[$_GET['selectedCS'][$curIndex]][$i];
...
//以下：获取halfA,halfB的值
foreach ($_GET["selectedCS"] as $courseId) {
    foreach ($_GET[$courseId] as $lessonId) {
...
while (true) {
    searchLesson(0, $_GET['spare'], $_GET['spare'], 0);
    if ($planCount == 0) {
